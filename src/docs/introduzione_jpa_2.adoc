...
In caso si voglia avere più istanza di una tipologia `@Embdedd` all'interno di
una entity occorre rinominarne le colonne (almeno di n-1):
...
== Struttura per @Embedded multiple

E su la struttura della tabella `person` sarà:

|===
|id |... |home_street |home_city |home_country |work_street |work_city |work_country
|===
